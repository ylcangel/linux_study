...
! 原作者linus，注释作者:sp00f
! 实模式分段模式
! 这个程序之所以这样是和历史有关，早期8080和兼容的8086，cpu是16位的，8086cpu的数据总线是16位，
! 2^16次方是64k，因此最大寻址范围是64k，程序被限制在64k字节(65535)的段内运行，也是段的最大长度。
! 段是从段落边界开始的，每个段落16字节大小，也就是说两个段至少相隔16字节；但8086cpu地址总线是20位
！可寻址2^20即1m大小空间（有20根地址线），为了解决16bit寻址20bit的问题，intel cpu设计出分段寻址，利用两个寄存器。
! 实模式下和保护模式下，cpu对分段寻址方式有所不同，实模式寻址方式是：段基址（段寄存器值<<4） + 偏移
！保护模式是：段选择符 + 偏移
！
...
! 
! 0x1000(<<4) * 0x10 = 0x10000 即 65536，即下面的程序被加载到第0x1000个段落所在的段中，地址是0x10000
...
ok_load:
	...
	sub	si,si	        ! si源变址寄存器，一般与段寄存器DS联用, 源地址ds:si
	sub	di,di	        ! di目地变址寄存器，一般与段寄存器ES联用, 目的地址es:di
	rep			        ! 前缀指令，有调用约定
